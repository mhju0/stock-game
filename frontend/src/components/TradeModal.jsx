        ...
        {loading ? (
          ...
          <>
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{ticker} · {stock.market}</div>
            </div>
            ...
            <div style={{
              ...
              <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{t('stock.price')}</div>
              <div style={{ fontSize: 28, fontWeight: 700 }}>{fmt(stock.price)}</div>
            </div>

            <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
              {[1, 5, 10, 25].map(v => (
                <button key={v} className="btn" onClick={() => setQuantity(v)} style={{
                  flex: 1, fontSize: 13, padding: '8px 0',
                  background: quantity === v ? 'var(--text-primary)' : 'transparent',
                  color: quantity === v ? 'var(--bg-primary)' : 'var(--text-primary)',
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button className="btn btn-buy" style={{ flex: 1 }} onClick={buy}>{t('stock.buy')}</button>
              <button className="btn btn-sell" style={{ flex: 1 }} onClick={sell}>{t('stock.sell')}</button>
            </div>

            {message && (
              <p style={{
                marginTop: 12, textAlign: 'center', fontSize: 14,
                color: message.includes('완료') || message.includes('complete') ? '#34c759' : '#ff3b30',
              }}>{message}</p>
            )}
          </>
        )}
